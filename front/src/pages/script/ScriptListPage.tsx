...
import {
  Box, Tab, Tabs, TextField, Button, Chip, Stack, MenuItem,
...
const SCRIPT_TYPES = ['介绍款', '爆款', '促单款', '种草款', '互动款']
...
function ScriptLibraryTab() {
  ...
  const [search, setSearch] = useState({ page: 0, rows: 20, keyword: '', scriptType: '', industry: '' })
  ...
  const openEdit = useCallback((row: ScriptItem) => {
    setForm({ id: row.id, title: row.title, content: row.content, category: row.category, tags: row.tags, scriptType: row.scriptType, industry: row.industry, duration: row.duration, status: row.status })
    setFormOpen(true)
  }, [])
  ...
  const handleBatchCheck = async () => {
    ...
    try {
      const res = await scriptApi.violationCheckBatch(selectedIds.map(id => String(id)))
  ...
  const highCount = violations.filter(v => (v as ComplianceViolation).severity === 1 || (v as ViolationWord).severity === 1).length
  const midCount = violations.filter(v => (v as ComplianceViolation).severity === 2 || (v as ViolationWord).severity === 2).length
  const lowCount = violations.filter(v => (v as ComplianceViolation).severity === 3 || (v as ViolationWord).severity === 3).length
  ...
  const columns: GridColDef[] = [
    ...
    { field: 'industry', headerName: '场景', width: 100,
      renderCell: ({ value }) => value ? <Chip label={value} size="small" variant="outlined" color="secondary" /> : null },
  ...
  const searchSlot = (
    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, p: 1, alignItems: 'center', bgcolor: 'var(--color-surface-dark)' }}>
      ...
      <TextField select size="small" label="类型" value={query.scriptType} onChange={e => setQuery(q => ({ ...q, scriptType: e.target.value, page: 0 }))} sx={{ width: 110, '& .MuiOutlinedInput-root': { borderRadius: 'var(--border-radius-lg)', bgcolor: 'var(--color-surface)', color: 'var(--color-text-primary)', '& fieldset': { borderColor: 'var(--color-surface-light)' }, '&:hover fieldset': { borderColor: 'var(--color-primary)' } } }}>
        <MenuItem value="">全部</MenuItem>
        {SCRIPT_TYPES.map(t => <MenuItem key={t} value={t}>{t}</MenuItem>)}
      </TextField>
      <TextField select size="small" label="场景" value={query.industry} onChange={e => setQuery(q => ({ ...q, industry: e.target.value, page: 0 }))} sx={{ width: 110, '& .MuiOutlinedInput-root': { borderRadius: 'var(--border-radius-lg)', bgcolor: 'var(--color-surface)', color: 'var(--color-text-primary)', '& fieldset': { borderColor: 'var(--color-surface-light)' }, '&:hover fieldset': { borderColor: 'var(--color-primary)' } } }}>
        <MenuItem value="">全部</MenuItem>
        {SCRIPT_SCENES.map(s => <MenuItem key={s} value={s}>{s}</MenuItem>)}
      </TextField>
      <Button size="small" variant="contained" onClick={() => setSearch(query)} sx={{ bgcolor: 'var(--color-primary)', color: '#000', '&:hover': { bgcolor: 'var(--color-primary-dark)' } }}>搜索</Button>
      <Box sx={{ flex: 1 }} />
      ...
      <Button size="small" variant="contained" startIcon={<AddIcon />} onClick={() => { setForm({}); setFormOpen(true) }} sx={{ bgcolor: 'var(--color-primary)', color: '#000', '&:hover': { bgcolor: 'var(--color-primary-dark)' } }}>新增话术</Button>
    </Box>
  )

  return (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', gap: 2, bgcolor: 'var(--color-surface-dark)', p: 'var(--spacing-lg)' }}>
      <StandardDataGrid
        rows={data?.list ?? []}
        columns={columns}
      ...
      {/* Preview Drawer */}
      <Drawer anchor="right" open={!!previewScript} onClose={() => setPreviewScript(null)} PaperProps={{ sx: { width: 560, bgcolor: 'var(--color-surface)', borderLeft: '1px solid var(--color-surface-light)' } }}>
        {previewScript && (
          <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
            <Box sx={{ display: 'flex', alignItems: 'center', p: 2, borderBottom: '1px solid var(--color-surface-light)' }}>
              ...
              <IconButton size="small" onClick={() => setPreviewScript(null)}><CloseIcon /></IconButton>
            </Box>
            ...
            {previewTab === 0 && (
              <Box sx={{ flex: 1, overflow: 'auto', p: 2 }}>
                <Stack direction="row" spacing={1} mb={1}>
                  {previewScript.scriptType && <Chip label={previewScript.scriptType} size="small" />}
                  {previewScript.industry && <Chip label={previewScript.industry} size="small" color="secondary" />}
                </Stack>
              ...
              <Box sx={{ flex: 1, overflow: 'auto', p: 2 }}>
                ...
                {!complianceResult && !complianceLoading && (
                  <Box textAlign="center" py={4}>
                    <Button variant="contained" onClick={() => handleRunCompliance(previewScript)}>开始检测</Button>
                  </Box>
                )}
                {complianceResult && (
                  <Box mt={2}>
      ...
      {/* Batch Check Dialog */}
      <Dialog open={batchCheckOpen} onClose={() => setBatchCheckOpen(false)} maxWidth="md" fullWidth PaperProps={{ sx: { bgcolor: 'var(--color-surface)', color: 'var(--color-text-primary)' } }}>
      ...
      <FormDialog open={formOpen} onClose={() => setFormOpen(false)} title={form.id ? '编辑话术' : '新增话术'}
        ...
        <Stack spacing={2}>
          ...
          <Grid container spacing={2}>
            <Grid item xs={6}>
              <TextField select label="类型" value={form.scriptType ?? ''} onChange={e => setForm(f => ({ ...f, scriptType: e.target.value }))} fullWidth>
                <MenuItem value="">请选择</MenuItem>
                {SCRIPT_TYPES.map(t => <MenuItem key={t} value={t}>{t}</MenuItem>)}
              </TextField>
            </Grid>
            <Grid item xs={6}>
              <TextField select label="场景" value={form.industry ?? ''} onChange={e => setForm(f => ({ ...f, industry: e.target.value }))} fullWidth>
                <MenuItem value="">请选择</MenuItem>
                {SCRIPT_SCENES.map(s => <MenuItem key={s} value={s}>{s}</MenuItem>)}
              </TextField>
            </Grid>
      ...
      {/* Delete Confirm */}
      <ConfirmDialog open={deleteId !== null} onClose={() => setDeleteId(null)} title="确认删除" content="此话术将被永久删除。"
...
function ViolationWordTab() {
  ...
  const { data, isFetching } = useQuery({
    ...
    queryFn: () => scriptApi.violationList({ ...search, severity: search.severity !== '' ? Number(search.severity) : undefined })
  })
  ...
  const toggleMut = useMutation({ mutationFn: ({ id }: { id: number; _enabled: boolean }) => scriptApi.violationToggle(id), onSuccess: () => qc.invalidateQueries({ queryKey: ['violation-words'] }), onError: (e: Error) => toast(e.message, 'error') })
  ...
  const columns: GridColDef[] = [
    ...
    { field: 'severity', headerName: '危险等级', width: 100,
      renderCell: ({ value }) => <Chip label={SEVERITY_LABELS[value as number] ?? '未知'} color={SEVERITY_COLORS[value as number] ?? 'default'} size="small" /> },
    ...
    { field: 'status', headerName: '启用', width: 80,
      renderCell: ({ row }: GridRenderCellParams) => (
        <Switch size="small" checked={(row as ViolationWord).status === 1}
          onChange={e => toggleMut.mutate({ id: (row as ViolationWord).id, _enabled: e.target.checked })} />
      ) },
  ...
  const searchSlot = (
    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, p: 1, alignItems: 'center', bgcolor: 'var(--color-surface-dark)' }}>
      ...
      <Box sx={{ flex: 1 }} />
      <Button size="small" variant="contained" startIcon={<AddIcon />} onClick={() => { setForm({}); setFormOpen(true) }} sx={{ bgcolor: 'var(--color-primary)', color: '#000', '&:hover': { bgcolor: 'var(--color-primary-dark)' } }}>新增违规词</Button>
    </Box>
  )
    ...
    <Box sx={{ display: 'flex', gap: 2, height: '100%' }}>
      ...
      <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 2 }}>
        <Paper variant="outlined" sx={{ p: 2, bgcolor: 'var(--color-surface)', borderColor: 'var(--color-surface-light)' }}>
          ...
          <Button sx={{ mt: 1, bgcolor: 'var(--color-primary)', color: '#000', '&:hover': { bgcolor: 'var(--color-primary-dark)' } }} variant="contained" fullWidth onClick={handleCheck} disabled={checkLoading}>
                ...
                : checkResult.violations.map((v, i) => (
                    <Paper key={i} variant="outlined" sx={{ p: 1, mb: 1, bgcolor: 'var(--color-surface-dark)', borderColor: 'var(--color-surface-light)' }}>
                      <Stack direction="row" spacing={1} alignItems="center" flexWrap="wrap">
                        <Chip label={SEVERITY_LABELS[v.severity] ?? '未知'} color={SEVERITY_COLORS[v.severity] ?? 'default'} size="small" />
      ...
      <FormDialog open={formOpen} onClose={() => setFormOpen(false)} title={form.id ? '编辑违规词' : '新增违规词'}
        ...
        <Stack spacing={2}>
          ...
          <TextField select label="危险等级" value={form.severity ?? 1} onChange={e => setForm(f => ({ ...f, severity: Number(e.target.value) }))} fullWidth>
            <MenuItem value={1}>高危</MenuItem>
            <MenuItem value={2}>中危</MenuItem>
            <MenuItem value={3}>低危</MenuItem>
          </TextField>
...
function ScriptTemplateTab() {
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, bgcolor: 'var(--color-surface-dark)', p: 'var(--spacing-lg)' }}>
      {/* Search bar */}
      <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <Box sx={{ flex: 1 }} />
        <Button size="small" variant="contained" startIcon={<AddIcon />} onClick={() => { setForm({}); setFormOpen(true) }} sx={{ bgcolor: 'var(--color-primary)', color: '#000', '&:hover': { bgcolor: 'var(--color-primary-dark)' } }}>新增模板</Button>
      </Box>
      ...
      <Dialog open={!!useDialog} onClose={() => setUseDialog(null)} maxWidth="sm" fullWidth PaperProps={{ sx: { bgcolor: 'var(--color-surface)', color: 'var(--color-text-primary)' } }}>
        ...
        <DialogActions sx={{ bgcolor: 'var(--color-surface-light)' }}>
          ...
          <Button variant="contained" onClick={applyTemplate} sx={{ bgcolor: 'var(--color-primary)', color: '#000', '&:hover': { bgcolor: 'var(--color-primary-dark)' } }}>填充并复制</Button>
        </DialogActions>
      </Dialog>
...
function HybridSearchTab() {
  ...
  return (
    <Box sx={{ display: 'flex', gap: 2, bgcolor: 'var(--color-surface-dark)', p: 'var(--spacing-lg)', borderRadius: 'var(--border-radius-xl)' }}>
      {/* Left: search panel */}
        ...
        <Paper variant="outlined" sx={{ p: 2, bgcolor: 'var(--color-surface)', borderColor: 'var(--color-surface-light)' }}>
          ...
          <Stack spacing={0.5} mb={2}>
            {(['hybrid', 'semantic', 'keyword'] as const).map(m => (
              <Box key={m} sx={{ display: 'flex', alignItems: 'center', cursor: 'pointer', py: 0.5 }}
                onClick={() => setMode(m)}>
      ...
      <Box sx={{ flex: 1 }}>
        ...
        <Box sx={{ position: 'relative', mb: 2 }}>
          <TextField fullWidth size="small" placeholder="输入关键词搜索..."
            ...
            InputProps={{
              ...
              endAdornment: <InputAdornment position="end"><Button size="small" variant="contained" onClick={() => doSearch()} sx={{ mr: -1, bgcolor: 'var(--color-primary)', color: '#000', '&:hover': { bgcolor: 'var(--color-primary-dark)' } }}>搜索</Button></InputAdornment>
            }} />
        ...
        {query && !isFetching && results.length === 0 && (
          <Typography sx={{ color: 'var(--color-text-secondary)', textAlign: 'center', py: 4 }}>未找到相关结果</Typography>
        )}
        <Stack spacing={1.5}>
          {results.map((item, idx) => {
            ...
            return (
              <Paper key={item.id ?? idx} variant="outlined" sx={{ p: 2, bgcolor: 'var(--color-surface)', borderColor: 'var(--color-surface-light)' }}>
...
export default function ScriptListPage() {
  ...
  return (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', bgcolor: 'var(--color-surface-dark)' }}>
      <Box sx={{ borderBottom: '1px solid var(--color-surface-light)', mb: 2, bgcolor: 'var(--color-surface)' }}>
        <Tabs value={tab} onChange={(_, v) => setTab(v)} sx={{ px: 'var(--spacing-lg)', pt: 'var(--spacing-md)' }}>
          <Tab label="话术库" />
          <Tab label="违规词管理" />
          <Tab label="模板库" />
          <Tab label="混合搜索" />
