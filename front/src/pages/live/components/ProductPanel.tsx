...
export function ProductPanel({
  ...
  return (
    <>
      <Card variant="outlined" sx={{ width: 300, flexShrink: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <CardContent sx={{ py: 1.5, '&:last-child': { pb: 1.5 } }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
            <Typography variant="subtitle2">选品 ({products.length})</Typography>
            <Button size="small" variant="contained" startIcon={<AddIcon />} onClick={() => onAddProductOpen(true)}>
              添加
            </Button>
          </Box>
        </CardContent>
                ...
                return (
                  ...
                  >
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                      <Tooltip title="上移">
                        <span>
                          <IconButton size="small" disabled={sorting || idx === 0} onClick={() => onMoveProduct(row, 'up')}>
                      ...
                      <Tooltip title="移除">
                        <IconButton size="small" color="error" onClick={() => onRemoveProduct(row)}>
      ...
      <Dialog open={addProductOpen} onClose={() => { onAddProductOpen(false); onSelectProductToAdd(null) }} maxWidth="sm" fullWidth>
            ...
            <>
              ...
              <List sx={{ maxHeight: 320, overflow: 'auto' }}>
                {productList
                  ...
                  .map((p) => (
                    <ListItemButton key={String(p.id)} onClick={() => onAddProduct(p)} disabled={addProductTypeLoading}>
                      <ListItemText primary={String(p.productName ?? p.id)} secondary={p.price != null ? `¥${Number(p.price).toFixed(2)}` : ''} />
                    </ListItemButton>
                  ))}
                {productList.filter((p) => !addedProductIds.has(p.id)).length === 0 && (
        ...
        <DialogActions>
          {selectedProductToAdd ? (
            <>
              <Button onClick={() => onSelectProductToAdd(null)}>返回</Button>
              <Button variant="contained" onClick={onConfirmAddProduct}>
                确认添加
              </Button>
            </>
          ) : (
            <Button onClick={() => onAddProductOpen(false)}>关闭</Button>
          )}
        </DialogActions>
      </Dialog>
    </>
