...
export function AiChatPanel({
  ...
  return (
    <Card variant="outlined" sx={{ width: 400, flexShrink: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <CardContent sx={{ py: 1.5, '&:last-child': { pb: 1.5 }, display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1 }}>
          ...
          {chatHistoryLength > 0 && (
            <Button size="small" onClick={onHistoryClear}>清空对话</Button>
          )}
        </Box>
        ...
        {chatResponse && (
          <Box sx={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', mb: 1 }}>
            ...
            <Box sx={{ display: 'flex', gap: 0.5, mt: 1, flexWrap: 'wrap' }}>
              <Button size="small" variant="contained" startIcon={<CheckIcon />} onClick={onApply}>
                应用到编辑区（可微调后保存）
              </Button>
              <Button
                size="small"
