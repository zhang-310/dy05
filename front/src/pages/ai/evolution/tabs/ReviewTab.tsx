...
export function ReviewTab() {
  ...
  const approveMut = useMutation({
    ...
    onSuccess: () => { toast('已通过', 'success'); qc.invalidateQueries({ queryKey: ['evolution-review'] }) },
    ...
    <Box>
      ...
      {statsData && (
        <Stack direction="row" spacing={2} mb={2}>
          {[
            { k: 'pendingCount', k2: 'pending', l: '待审核', c: '#f57c00' },
            { k: 'approvedCount', k2: 'approved', l: '已通过', c: '#388e3c' },
            { k: 'rejectedCount', k2: 'rejected', l: '已拒绝', c: '#d32f2f' },
          ].map(s => (
            <Paper key={s.k} variant="outlined" sx={{ p: 1.5, minWidth: 100, textAlign: 'center', bgcolor: 'var(--color-surface)', borderColor: 'var(--color-surface-light)' }}>
              <Typography variant="h5" fontWeight={700} sx={{ color: s.c }}>{stat(s.k) || stat(s.k2)}</Typography>
