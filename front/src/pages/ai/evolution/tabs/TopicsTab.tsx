...
export function TopicsTab({ scopeKbId }: TopicsTabProps) {
  ...
  const saveMut = useMutation({
    ...
    onSuccess: () => { toast('保存成功', 'success'); setEditItem(null); qc.invalidateQueries({ queryKey: ['evolve-topics'] }) },
    onError: () => toast('保存失败', 'error'),
  })
  const deleteMut = useMutation({
    mutationFn: (id: number) => aiApi.topicDelete(id),
    onSuccess: () => { toast('已删除', 'success'); qc.invalidateQueries({ queryKey: ['evolve-topics'] }) },
    onError: () => toast('删除失败', 'error'),
  })
  const triggerMut = useMutation({
    mutationFn: (id: number) => aiApi.pendingDeepenTrigger(id),
    onSuccess: () => toast('深度进化已触发', 'success'),
        ...
        return (
          <Stack direction="row" spacing={0.5}>
            ...
            <Button size="small" startIcon={<PlayArrowIcon />}
              onClick={() => triggerMut.mutate(Number(row.id))}
              disabled={triggerMut.isPending}>运行</Button>
            <IconButton size="small" color="error" onClick={() => deleteMut.mutate(Number(row.id))}>
    ...
    <Box sx={{ bgcolor: 'var(--color-surface-dark)', p: 'var(--spacing-lg)', borderRadius: 'var(--border-radius-xl)' }}>
      {isError ? (
        <Alert severity="error" sx={{ mb: 2 }}>
          加载主题池失败：{error instanceof Error ? error.message : '请检查是否已登录或网络/后端是否正常'}
        </Alert>
      ) : null}
      ...
      <Stack direction="row" justifyContent="flex-end" mb={2}>
        <Button variant="contained" startIcon={<AddIcon />} onClick={() => setEditItem({
          status: 1,
          priority: 2,
          kbId: scopeKbNum,
        })}
          sx={{ bgcolor: 'var(--color-primary)', color: '#000', '&:hover': { bgcolor: 'var(--color-primary-dark)' } }}>新建主题</Button>
      </Stack>
      {/*
      ...
      <Dialog open={!!editItem} onClose={() => setEditItem(null)} maxWidth="sm" fullWidth>
        ...
        <DialogActions sx={{ bgcolor: 'var(--color-surface-light)' }}>
          ...
          <Button variant="contained" disabled={saveMut.isPending}
            onClick={submitSave}
            sx={{ bgcolor: 'var(--color-primary)', color: '#000', '&:hover': { bgcolor: 'var(--color-primary-dark)' } }}>保存</Button>
        </DialogActions>
      </Dialog>
    </Box>
  )
