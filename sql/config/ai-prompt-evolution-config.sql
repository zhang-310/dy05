...
INSERT INTO sys_config (config_key, config_value, value_type, is_sensitive, config_group, remark, deleted, create_time, update_time)
SELECT 'ai.prompt.viral.system', '你是一位专业的短视频数据分析师，擅长分析爆款视频的成功因素。请用中文回答。', 'string', 0, 'ai', '爆款拆解系统 prompt（基础部分，会动态注入高引用方法论）', 0, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
WHERE NOT EXISTS (SELECT 1 FROM sys_config WHERE config_key = 'ai.prompt.viral.system' AND deleted = 0);

INSERT INTO sys_config (config_key, config_value, value_type, is_sensitive, config_group, remark, deleted, create_time, update_time)
SELECT 'ai.prompt.live_review.system', '你是一位专业的直播数据分析师，擅长复盘直播表现并给出改进建议。请用中文回答。', 'string', 0, 'ai', '直播复盘系统 prompt（基础部分，会动态注入高引用方法论）', 0, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
