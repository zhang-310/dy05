...
public class EvolutionController {
    ...
    public RESTResult<Map<String, Object>> evolveDashboardStatus(HttpServletRequest request) {
        ...
        try {
            ...
            boolean circuitOpen = Boolean.TRUE.equals(breaker.get("open"));
            Map<String, Object> data = new LinkedHashMap<>();
            data.put("running", running);
            data.put("circuitBroken", circuitOpen);
            data.put("circuitReason", breaker.get("reason"));
            if (circuitOpen) {
                data.put("message", "进化引擎已暂停：LLM 账号余额不足或额度异常，请充值/切换可用模型后再恢复调度。");
            }
    ...
    public RESTResult<Map<String, Object>> executionExecute(
        ...
        try {
            if (evolveCircuitBreakerService.isOpen()) {
                return RESTResult.error(ErrorCode.OPERATION_NOT_ALLOWED,
                        "进化引擎已暂停: " + evolveCircuitBreakerService.currentReason());
            }
