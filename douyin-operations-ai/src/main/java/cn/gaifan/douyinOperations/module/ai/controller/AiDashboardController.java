...
public class AiDashboardController {
    ...
    @Operation(summary = "触发知识库文档质量重评估")
    @PostMapping("/kb-quality/rescan")
    public RESTResult<Map<String, Object>> rescanKnowledgeQuality(
            @RequestBody(required = false) Map<String, Object> body,
            HttpServletRequest request
    ) {
        requireAdmin(request);
        int limit = body != null && body.get("limit") instanceof Number n ? n.intValue() : 1000;
        int updated = kbDocumentQualityService.scanUnevaluated(limit);
        boolean completed = updated < 0;
        return RESTResult.getSuccess(Map.of(
                "updated", completed ? 0 : updated,
                "completed", completed
        ));
    }
