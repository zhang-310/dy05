...
import cn.gaifan.douyinOperations.module.ai.util.DeterministicEmbeddingGenerator;
...
public class VectorServiceImpl implements VectorService {
    ...
    @Value("${app.ai.kb.stub-embedding:false}")
    private boolean stubEmbedding;
    ...
    public List<Float> generateEmbedding(String text) {
        if (stubEmbedding) {
            return DeterministicEmbeddingGenerator.generate(text, DeterministicEmbeddingGenerator.DEFAULT_DIMENSION);
        }
        ...
        for (int attempt = 0; attempt < EMBEDDING_MAX_RETRIES; attempt++) {
            try {
                ...
                if (stringRedisTemplate != null && result != null && !result.isEmpty()) {
                    stringRedisTemplate.opsForValue().set(cacheKey, JSON.toJSONString(result), Math.max(1, embeddingCacheTtlDays), TimeUnit.DAYS);
                }
                return result;
            } catch (Exception e) {
                lastEx = e;
    ...
    private boolean isRetryableEmbeddingError(Exception e) {
        ...
        while (cur != null && depth < 8) {
            ...
            String msg = cur.getMessage();
            String haystack = (className + " " + simpleName + " " + (msg != null ? msg : ""))
                    .toLowerCase(Locale.ROOT);
            if (haystack.contains("connectexception")
                    || haystack.contains("closedchannelexception")
                    || haystack.contains("sockettimeoutexception")
                    || haystack.contains("httptimeoutexception")
                    || haystack.contains("socketexception")
                    || haystack.contains("eofexception")
                    || haystack.contains("http 408")
                    || haystack.contains("http 429")
                    || haystack.contains("http 500")
