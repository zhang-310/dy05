...
public class AiAdminInfraController {
    ...
    @Value("${app.gaifan.allow-header-identity:true}")
    ...
    public RESTResult<Map<String, Object>> getMonitoringConfig(@RequestBody(required = false) Map<String, Object> body, HttpServletRequest request) {
        ...
        m.put("creditEnforce", creditEnforce);
        m.put("allowHeaderIdentity", allowHeaderIdentity);
    ...
    @Operation(summary = "Redis 缓存诊断（命中率、Key 分布、TTL 分布、优化建议）")
    @PostMapping("/cache/diagnostics")
    public RESTResult<Map<String, Object>> getCacheDiagnostics(@RequestBody(required = false) Map<String, Object> body, HttpServletRequest request) {
        requireAdmin(request);
        return RESTResult.getSuccess(aiAdminInfraService.getCacheDiagnostics());
    }
