...
public interface AiKbDocumentRepository extends JpaRepository<AiKbDocument, Long>, JpaSpecificationExecutor<AiKbDocument> {
    ...
    @Query("SELECT COUNT(d) FROM AiKbDocument d JOIN AiKnowledgeBase k ON d.kbId = k.id "
            + "WHERE d.deleted = 0 AND k.deleted = 0 AND k.kbName = :kbName")
    long countByKbName(@Param("kbName") String kbName);

    @Query("SELECT MAX(d.createTime) FROM AiKbDocument d JOIN AiKnowledgeBase k ON d.kbId = k.id "
            + "WHERE d.deleted = 0 AND k.deleted = 0 AND k.kbName = :kbName")
    Timestamp findLastCreateTimeByKbName(@Param("kbName") String kbName);
    ...
    /** P2 质量启发扫描：优先补齐未评估文档 */
    @Query("SELECT d FROM AiKbDocument d WHERE d.deleted = 0 AND d.qualityHeuristicScore IS NULL ORDER BY d.id ASC")
    List<AiKbDocument> findUnevaluatedQualityDocuments(Pageable pageable);

    /** AI 中心仪表盘：知识库文档质量均分（仅统计已评估文档） */
    @Query("SELECT AVG(d.qualityHeuristicScore) FROM AiKbDocument d WHERE d.deleted = 0 AND d.qualityHeuristicScore IS NOT NULL")
    Double calculateAverageQualityHeuristicScore();

    /** AI 中心仪表盘：知识库文档总数 */
    long countByDeleted(Integer deleted);

    /** AI 中心仪表盘：已完成质量评估的文档数 */
    @Query("SELECT COUNT(d) FROM AiKbDocument d WHERE d.deleted = 0 AND d.qualityHeuristicScore IS NOT NULL")
    long countEvaluatedQualityDocuments();

    /** AI 中心仪表盘：未完成质量评估的文档数 */
    @Query("SELECT COUNT(d) FROM AiKbDocument d WHERE d.deleted = 0 AND d.qualityHeuristicScore IS NULL")
    long countUnevaluatedQualityDocuments();

    /** AI 中心仪表盘：低于阈值的已评估文档数 */
    @Query("SELECT COUNT(d) FROM AiKbDocument d WHERE d.deleted = 0 AND d.qualityHeuristicScore IS NOT NULL AND d.qualityHeuristicScore < :threshold")
    long countQualityScoreBelow(@Param("threshold") int threshold);
    ...
    /** 官方采集：按来源 URL 定位旧文档，便于补齐媒体字段后重建索引 */
    @Query("SELECT d FROM AiKbDocument d WHERE d.kbId = :kbId AND d.sourceType = :sourceType AND d.deleted = 0 AND d.metadata LIKE :sourceUrlPattern ORDER BY d.id DESC")
    List<AiKbDocument> findByKbSourceTypeAndMetadataSourceUrl(@Param("kbId") Long kbId,
                                                              @Param("sourceType") String sourceType,
                                                              @Param("sourceUrlPattern") String sourceUrlPattern,
                                                              Pageable pageable);
    ...
    /** P0 双写补偿：status=0 且超时未完成，sync_retry_count 低于配置上限 */
    @Query("SELECT d FROM AiKbDocument d WHERE d.deleted = 0 AND d.status = 0 AND (d.syncRetryCount IS NULL OR d.syncRetryCount < :maxRetryCount) AND d.createTime < :cutoff ORDER BY d.createTime ASC")
    List<AiKbDocument> findStuckForCompensation(@Param("cutoff") Timestamp cutoff,
