...
public class AiDashboardServiceImpl implements AiDashboardService {
    ...
    private AiKbDocumentRepository kbDocumentRepository;
    ...
    public Map<String, Object> getDashboardStats() {
        ...
        // 知识库质量均分：这里统计真实 KB 文档启发式质量，而不是直播话术质量历史表。
        double qualityScore = 0.0;
        long qualityDocCount = 0L;
        long evaluatedQualityDocCount = 0L;
        long unevaluatedQualityDocCount = 0L;
        long lowQualityDocCount = 0L;
        if (kbDocumentRepository != null) {
            qualityDocCount = kbDocumentRepository.countByDeleted(0);
            evaluatedQualityDocCount = kbDocumentRepository.countEvaluatedQualityDocuments();
            unevaluatedQualityDocCount = kbDocumentRepository.countUnevaluatedQualityDocuments();
            lowQualityDocCount = kbDocumentRepository.countQualityScoreBelow(60);
            Double avg = kbDocumentRepository.calculateAverageQualityHeuristicScore();
            qualityScore = avg != null ? avg : 0.0;
        }
        stats.put("avgQualityScore", Math.round(qualityScore * 10) / 10.0);
        stats.put("qualityDocCount", qualityDocCount);
        stats.put("evaluatedQualityDocCount", evaluatedQualityDocCount);
        stats.put("unevaluatedQualityDocCount", unevaluatedQualityDocCount);
        stats.put("lowQualityDocCount", lowQualityDocCount);
        double qualityEvaluationCoverage = qualityDocCount > 0 ? evaluatedQualityDocCount * 100.0 / qualityDocCount : 0.0;
        stats.put("qualityEvaluationCoverage", Math.round(qualityEvaluationCoverage * 10) / 10.0);
