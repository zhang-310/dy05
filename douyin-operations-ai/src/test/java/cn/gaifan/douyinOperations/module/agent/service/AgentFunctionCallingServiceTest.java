...
class AgentFunctionCallingServiceTest {
    ...
    private AgentMessage message(Long id, Integer senderType, String content) {
    ...
    @Test
    void execute_shouldFallbackToPlainChat_whenDefaultModelIsOllamaAndToolsConfigured() {
        // Given
        String systemPrompt = "你是智能助手";
        String userMessage = "你好";
        testModel.setModelProvider("ollama");
        testModel.setModelVersion("qwen2.5:7b");
        ...
        LlmClient.LlmResponse plainResponse = new LlmClient.LlmResponse(
            "OK，普通对话已回复", 20, true, null
        );
        when(llmClient.chat(any(AiModel.class), anyString(), anyString())).thenReturn(plainResponse);
        ...
        // Then
        assertThat(result.success).isTrue();
        assertThat(result.content).contains("普通对话已回复");
        assertThat(result.toolCalls).isEmpty();
        verify(llmClient).chat(any(AiModel.class), anyString(), anyString());
        verify(llmClient, never()).chatWithToolsStructured(any(AiModel.class), anyList(), anyString());
    }

    @Test
    void execute_shouldInjectRecentConversationHistory_withoutDuplicatingCurrentUserMessage() {
        // Given
        String systemPrompt = "你是智能助手";
        String userMessage = "继续说";
        ...
        AgentMessage oldUser = message(1L, 1, "上一轮问题");
        AgentMessage oldAssistant = message(2L, 2, "上一轮回答");
        AgentMessage currentPersisted = message(3L, 1, userMessage);
        when(agentMessageRepository.findTop20ByConversationIdAndDeletedOrderByIdDesc(conversationId, 0))
                .thenReturn(List.of(currentPersisted, oldAssistant, oldUser));

        LlmClient.LlmToolResponse mockResponse = new LlmClient.LlmToolResponse(
                "结合上下文继续回答", 100, true, null, null, null
        );
        when(llmClient.chatWithToolsStructured(any(AiModel.class), anyList(), anyString()))
                .thenReturn(mockResponse);
        ...
        // Then
        assertThat(result.success).isTrue();
        verify(llmClient).chatWithToolsStructured(any(AiModel.class), argThat(messages -> {
            ...
            long currentCount = contents.stream().filter(c -> c.contains(userMessage)).count();
            return roles.equals(List.of("system", "user", "assistant", "user"))
                    && contents.stream().anyMatch(c -> c.contains("上一轮问题"))
                    && contents.stream().anyMatch(c -> c.contains("上一轮回答"))
                    && currentCount == 1;
        }), anyString());
    }

    @Test
    void execute_shouldKeepNewestHistoryFirst_whenHistoryExceedsBudget() {
        // Given
        String systemPrompt = "你是智能助手";
        String userMessage = "继续总结";
        ...
        AgentMessage currentPersisted = message(23L, 1, userMessage);
        AgentMessage newestAssistant = message(22L, 2, "最近一轮关键结论");
        AgentMessage newestUser = message(21L, 1, "最近一轮问题");
        AgentMessage oldHuge7 = message(7L, 1, "很早以前的超长消息-7" + "x".repeat(20000));
        AgentMessage oldHuge6 = message(6L, 2, "很早以前的超长消息-6" + "x".repeat(20000));
        AgentMessage oldHuge5 = message(5L, 1, "很早以前的超长消息-5" + "x".repeat(20000));
        AgentMessage oldHuge4 = message(4L, 2, "很早以前的超长消息-4" + "x".repeat(20000));
        AgentMessage oldHuge3 = message(3L, 1, "很早以前的超长消息-3" + "x".repeat(20000));
        AgentMessage oldHuge2 = message(2L, 2, "很早以前的超长消息-2" + "x".repeat(20000));
        AgentMessage oldHuge1 = message(1L, 1, "很早以前的超长消息-1" + "x".repeat(20000));
        when(agentMessageRepository.findTop20ByConversationIdAndDeletedOrderByIdDesc(conversationId, 0))
                .thenReturn(List.of(currentPersisted, newestAssistant, newestUser,
                        oldHuge7, oldHuge6, oldHuge5, oldHuge4, oldHuge3, oldHuge2, oldHuge1));

        LlmClient.LlmToolResponse mockResponse = new LlmClient.LlmToolResponse(
                "结合最近上下文回答", 100, true, null, null, null
        );
        when(llmClient.chatWithToolsStructured(any(AiModel.class), anyList(), anyString()))
                .thenReturn(mockResponse);
        ...
        verify(llmClient).chatWithToolsStructured(any(AiModel.class), argThat(messages -> {
            List<String> contents = messages.stream().map(m -> String.valueOf(m.get("content"))).toList();
            return contents.stream().anyMatch(c -> c.contains("最近一轮问题"))
                    && contents.stream().anyMatch(c -> c.contains("最近一轮关键结论"))
                    && contents.stream().noneMatch(c -> c.contains("很早以前的超长消息-1"));
        }), anyString());
    }

    @Test
    void execute_shouldResolveStableLowestIdModel_whenNoModelMarkedDefault() {
        // Given
        String systemPrompt = "你是智能助手";
        String userMessage = "你好";
        AiModel laterModel = new AiModel();
        laterModel.setId(28L);
        laterModel.setModelProvider("ollama");
        laterModel.setModelVersion("qwen2.5:7b");
        laterModel.setIsDefault(0);
        laterModel.setStatus(1);

        AiModel firstModel = new AiModel();
        firstModel.setId(1L);
        firstModel.setModelProvider("ollama");
        firstModel.setModelVersion("qwen3:8b");
        firstModel.setIsDefault(0);
        firstModel.setStatus(1);

        when(skillExecutor.getAvailableToolNames(agentId)).thenReturn(Collections.emptyList());
        when(aiModelRepository.findByStatusAndDeleted(1, 0)).thenReturn(List.of(laterModel, firstModel));

        LlmClient.LlmResponse plainResponse = new LlmClient.LlmResponse(
            "稳定模型已回复", 20, true, null
        );
        when(llmClient.chat(any(AiModel.class), anyString(), anyString())).thenReturn(plainResponse);
        ...
        // Then
        assertThat(result.success).isTrue();
        assertThat(result.content).contains("稳定模型已回复");
        verify(llmClient).chat(argThat(model -> model.getId().equals(1L)), anyString(), anyString());
    }
    ...
    @Test
    void execute_shouldPassReasoningContentBack_whenToolModelRequiresThinkingTrace() throws Exception {
        // Given
        String systemPrompt = "你是智能助手";
        String userMessage = "查看运营状态";

        when(skillExecutor.getAvailableToolNames(agentId)).thenReturn(List.of("douyin_ops_commander"));
        when(aiModelRepository.findByStatusAndDeleted(1, 0)).thenReturn(List.of(testModel));

        Skill mockSkill = mock(Skill.class);
        when(mockSkill.getName()).thenReturn("douyin_ops_commander");
        when(mockSkill.getDescription()).thenReturn("运营总控");
        when(mockSkill.execute(any(Skill.SkillContext.class))).thenReturn("队列正常");
        when(skillRegistry.findByName("douyin_ops_commander")).thenReturn(Optional.of(mockSkill));

        String toolCallsJson = "[{\"id\":\"call_1\",\"type\":\"function\",\"function\":{\"name\":\"douyin_ops_commander\",\"arguments\":\"{}\"}}]";
        String rawJson = "{\"choices\":[{\"message\":{\"content\":\"\",\"reasoning_content\":\"思考轨迹\",\"tool_calls\":[]}}],\"usage\":{\"total_tokens\":20}}";
        LlmClient.LlmToolResponse firstResponse = new LlmClient.LlmToolResponse(
            "", 20, true, null, toolCallsJson, rawJson
        );
        LlmClient.LlmToolResponse secondResponse = new LlmClient.LlmToolResponse(
            "已查看，队列正常。", 30, true, null, null, "{\"choices\":[{\"message\":{\"content\":\"已查看，队列正常。\"}}]}"
        );
        when(llmClient.chatWithToolsStructured(any(AiModel.class), anyList(), anyString()))
        ...
        // Then
        assertThat(result.success).isTrue();
        assertThat(result.content).contains("队列正常");
        verify(llmClient, atLeastOnce()).chatWithToolsStructured(any(AiModel.class), argThat(messages ->
            messages.stream().anyMatch(message -> "思考轨迹".equals(message.get("reasoning_content")))
        ), anyString());
    }
